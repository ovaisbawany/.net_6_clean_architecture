...
namespace Sample.Core.Generic
{
        ...
        where TKey : IEquatable<TKey>
    {
        ...
        public QueryFilter(IRepository<TEntity, TKey> repository)
        {
            if (repository == null)
            {
        ...
        public QueryFilter(IRepository<TEntity, TKey> repository, Expression<Func<TEntity, bool>> expression)
            : this(repository)
        {
        ...
        {
            return await _repository.GetPagedResultAsync(filter, orderBy, page, pageSize, includes);
        }
        ...
        {
            //return await _repository.SelectAsync(_expression, _orderBy, _includeInCore);
            return (await _repository.GetPagedResultAsync(_expression, _orderBy, null, null, _includeInCore)).Item2;
        }
    }
}
